    ...
    <section ref={ref} className="w-full py-8 md:py-12 px-4 flex justify-center bg-background">
      <motion.figure initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6, ease: "easeOut" }} className="w-full max-w-4xl">
        ...
        <figcaption className="mt-3 text-center">
          <button onClick={() => { const target = document.getElementById("market-baseline") || document.getElementById("truth-gate"); target?.scrollIntoView({ behavior: "smooth" }); }} className="text-sm text-primary hover:text-primary/80 transition-colors cursor-pointer font-extrabold md:text-xl" style={{ fontFamily: "'Jost', sans-serif" }}>
