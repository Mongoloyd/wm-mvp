    ...
    <section className="wm-ambient">
      <div ref={ref} className="mx-auto max-w-5xl px-4 md:px-8 py-20 md:py-28">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.15 }} className="text-center">
          <div className="mx-auto mb-6 w-10 h-[1px] bg-primary" />
          <p className="eyebrow mb-5">HERE'S WHAT NO CONTRACTOR WILL TELL YOU BEFORE YOU SIGN.</p>
          <h2 className="display-hero text-foreground mx-auto mb-4" style={{ fontSize: "clamp(34px, 5vw, 48px)", maxWidth: 700, lineHeight: 1.15 }}>
            THE IMPACT WINDOW INDUSTRY HAS NO PRICING STANDARD.
          </h2>
          <p className="wm-copy font-body text-[17px] mx-auto mb-14" style={{ maxWidth: 640, lineHeight: 1.75 }}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {blocks.map((block, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 24 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.15, delay: i * 0.05 }}>
              ...
              <p className="wm-copy font-body text-[15px] text-slate-700 mt-2 leading-[1.7]">{block.copy}</p>
              <span className="inline-block mt-3 badge-warning rounded-lg px-2.5 py-1 font-body text-[11px] font-semibold">{block.badge}</span>
            </motion.div>
          ))}
        </div>

        <motion.div initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.15, delay: 0.2 }}
          className="text-center mt-16 glass-card rounded-2xl p-10 md:p-12">
          <p className="display-secondary text-foreground mb-4" style={{ fontSize: "clamp(22px, 3vw, 30px)" }}>
            MOST HOMEOWNERS ASSUME THE QUOTE REFLECTS THE MARKET RATE.
          </p>
          <p className="display-hero text-wm-orange" style={{ fontSize: "clamp(22px, 3vw, 30px)" }}>IT USUALLY DOESN'T.</p>
        </motion.div>
        ...
        <div className="flex flex-col md:flex-row items-center justify-center gap-4 mt-12">
          ...
          {onDemoClick && (
            <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={onDemoClick}
              className="btn-depth-secondary rounded-xl font-body text-sm font-semibold py-3 px-7">
