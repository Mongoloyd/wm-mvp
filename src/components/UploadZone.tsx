  ...
  const handleScan = async () => {
    ...
    try {
      ...
      if (!leadId) {
        ...
        const { error: leadErr } = await supabase.from("leads").insert({ id: fallbackLeadId, session_id: fallbackSessionId, source: "direct_upload" });
        if (leadErr) { console.error("Failed to create fallback lead:", leadErr); toast.error("Failed to initialize session. Please try again."); setUploading(false); return; }
    ...
    <AnimatePresence>
      {isVisible && (
        <motion.div ref={containerRef} initial={{ opacity: 0, height: 0, y: 20 }} animate={{ opacity: 1, height: "auto", y: 0 }} exit={{ opacity: 0, height: 0 }} transition={{ duration: 0.15 }} className="overflow-hidden max-w-2xl mx-auto mt-6">
          <div className="glass-card-strong rounded-2xl p-8 md:p-10" style={{ boxShadow: "var(--wm-shadow-float)" }}>
            <span className="inline-block eyebrow badge-signal rounded-lg px-3 py-1 mb-5">UPLOAD YOUR QUOTE</span>
            <h2 className="display-secondary text-foreground mb-2" style={{ fontSize: 26 }}>Drop your quote to start the scan.</h2>
            <p className="wm-copy font-body text-[15px] mb-7">Upload your contractor's quote and we'll analyze it in under 60 seconds.</p>
            ...
            <div
              ...
              className={`border-2 border-dashed rounded-xl p-12 text-center cursor-pointer transition-all duration-150 ${isDragOver ? 'border-primary bg-primary/5' : 'border-border bg-muted/30'}`}
            >
              ...
              {!file ? (
                ...
                <div onClick={(e) => e.stopPropagation()}>
                  <div className="flex items-center justify-center mx-auto w-12 h-12 rounded-xl bg-primary/10 mb-3">
            ...
            {fileError && <p className="font-body text-[13px] text-wm-orange text-center mt-3 font-medium">{fileError}</p>}
            ...
            {file && (
              <motion.button initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.15 }} whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.98 }}
                ...
                className="w-full btn-depth-primary rounded-xl font-body text-[17px] font-bold mt-5 py-3.5"
