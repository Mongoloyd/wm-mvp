...
function Card({ children, style = {}, highlight = false }) {return <div style={{ background: T.card, border: `1px solid ${highlight ? "rgba(6,182,212,0.22)" : T.border}`, borderRadius: 0, padding: "20px", ...(highlight ? { boxShadow: "0 0 40px rgba(6,182,212,0.06)" } : {}), ...style }}>{children}</div>;}
...
function PowerToolButton({ onClick }) {const [hover, setHover] = useState(false);return <button onClick={onClick} onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)} className="w-full sm:w-auto py-4 px-6 sm:px-8" style={{ display: "inline-flex", alignItems: "center", gap: "10px", background: "#0891B2", color: "#FFFFFF", fontFamily: "'DM Sans', sans-serif", fontSize: 16, fontWeight: 700, borderRadius: 5, border: "none", boxShadow: "0 4px 14px rgba(8,145,178,0.35)", cursor: "pointer", whiteSpace: "nowrap", transform: hover ? "scale(1.02)" : "scale(1)", transition: "transform 0.15s ease, box-shadow 0.2s ease" }}><span style={{ flexShrink: 0, fontSize: 18 }}>✨</span><span style={{ flex: 1, textAlign: "left" }}>SEE THE AI IN ACTION</span><span style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", width: 28, height: 28, borderRadius: "50%", background: "rgba(255,255,255,0.2)", flexShrink: 0 }}>▶</span></button>;}
...
function LeadModal({ onComplete, onClose }) {
  ...
  return (
    <div onClick={(e) => e.target === e.currentTarget && onClose()} style={{ position: "fixed", inset: 0, zIndex: 9000, background: "rgba(0,0,0,0.82)", backdropFilter: "blur(10px)", display: "flex", alignItems: "center", justifyContent: "center", padding: "20px", fontFamily: "'Inter', system-ui, sans-serif" }}>
      <div style={{ width: "100%", maxWidth: "468px", background: "#0d1117", border: "1px solid rgba(6,182,212,0.18)", borderRadius: 0, overflow: "hidden", position: "relative", boxShadow: "0 0 0 1px rgba(255,255,255,0.04), 0 40px 90px rgba(0,0,0,0.65)" }}>
