          ...
          <div className="grid grid-cols-5 gap-6">
            {steps.map((step, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.15, delay: i * 0.05 }} className="relative text-center">
                ...
                <p className="font-body text-[13px] text-muted-foreground mt-1 leading-[1.65]">{step.copy}</p>
              </motion.div>
            ))}
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {deliverables.map((d, i) => (
              <div key={i} className="flex items-start gap-3">
                ...
                <p className="font-body text-[13px] text-muted-foreground leading-[1.65]">{d.text}</p>
              </div>
            ))}
          </div>
        </motion.div>
